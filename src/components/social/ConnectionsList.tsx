...
const ConnectionsList = ({ checkStatus, profileId, maxVisible }: ConnectionsListProps) => {
  const { user } = useAuth();
  ...
  if (!['accepted', 'pending', 'blocked'].includes(checkStatus)) return null;
  if (!connections) return null;

  const filteredConnections = connections.filter((conn) => {
    if (checkStatus === 'accepted') return conn.status === 'accepted';
    if (checkStatus === 'blocked') return conn.status === 'blocked';
  ...
  const getOtherProfileId = (conn: UserConnection) => {
    ...
    <div className="max-w-4xl mx-auto w-full bg-gradient-to-l from-gray-900 py-4 flex flex-wrap justify-center rounded-lg gap-4">
      <div className="w-full flex items-center justify-between px-4 mb-2">
        <h2 className="text-xl font-bold text-white">
          {title}
          {showCount && (
            <span className="ml-2 text-sky-400 text-lg font-semibold">
