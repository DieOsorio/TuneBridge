...
const AdCard: React.FC<AdCardProps> = ({ ad, publisherId }) => {
  ...
  return (
    <div
      className={`${isLooking ? "border-cyan-600" : "border-emerald-600"} rounded-2xl border sm:min-w-100 bg-gray-900 p-5 shadow-sm hover:shadow-md transition-shadow space-y-4`}
    >
      {/* Header */}
      <div className="flex items-center justify-between gap-4">
        <h3
          className={`text-lg text-left font-semibold ${
            isLooking ? "text-cyan-600" : "text-emerald-600"
          }`}
        >
          {ad.title}
        </h3>
        <span
          className={`text-xs font-semibold min-w-20 text-center px-2 py-1 rounded-lg leading-tight ${
            isLooking ? "bg-cyan-100 text-cyan-600" : "bg-emerald-100 text-emerald-600"
          }`}
        >
